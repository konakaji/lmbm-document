...
この統一された形状により、異なるサイズの分子を同じ機械学習モデルで効率的に処理できるようになっています。特徴次元18は、軌道寄与（14次元）と分子軌道エネルギー（4次元）の合計であり、E3NNで処理可能な形式に変換されています。
